...
if __name__ == '__main__':
    ...
    if args.data_group == 'NCI1':   # 80 clients
        EPS_1 = 0.04
        EPS_2 = 0.3
    if args.data_group == 'PROTEINS':   # 30 clients
        EPS_1 = 0.05
        EPS_2 = 0.25
    if args.data_group == 'IMDB-BINARY' and args.num_clients == 20:    # 20 or 30 clients
        EPS_1 = 0.04
        EPS_2 = 0.15
    if args.data_group == 'IMDB-BINARY' and args.num_clients == 10:    # 10 clients
        EPS_1 = 0.04
        EPS_2 = 0.10
    ...
    if args.overlap and args.standardize:
        outpath = os.path.join(outbase, f"standardizedDTW/oneDS-overlap-{args.num_clients}clients")
    elif args.overlap:
        outpath = os.path.join(outbase, f"oneDS-overlap-{args.num_clients}clients")
    elif args.standardize:
        outpath = os.path.join(outbase, f"standardizedDTW/oneDS-nonOverlap-{args.num_clients}clients")
    else:
        outpath = os.path.join(outbase, f"oneDS-nonOverlap-{args.num_clients}clients")

    Path(outpath).mkdir(parents=True, exist_ok=True)
    print(f"Output Path: {os.path.join(outpath, args.data_group)}")


    """ distributed one dataset to multiple clients, while keeping the same testing data in each client """
    ...
    preparedData = setupGC.prepareData_oneDS(datapath, args.data_group, args.batch_size, convert_x=args.convert_x, seed=seed_dataSplit)
    splitedData, df_stats = setupGC.distributeData_oneDS(preparedData[0], preparedData[1],
                                                         args.data_group, args.batch_size,
                                                         num_client=args.num_clients, seed=seed_dataSplit,
